...
import { Flange } from '../../types/flange';
 ...
 */
export function normaliseFlangeType(flangeType: string | null): string {
  if (!flangeType) return 'GEN';

  const normalized = flangeType.toUpperCase().trim();

  // Weld Neck variants
  if (normalized.includes('WELD NECK') || normalized === 'WN') {
    if (normalized.includes('RTJ')) return 'WNRTJ';
    if (normalized.includes('RF') || normalized.includes('RAISED')) return 'WNRF';
    if (normalized.includes('FF') || normalized.includes('FLAT')) return 'WNFF';
    return 'WNRF'; // Default to RF
  }

  // Slip-On variants
  if (normalized.includes('SLIP') || normalized.includes('SO')) {
    if (normalized.includes('RTJ')) return 'SORTJ';
    if (normalized.includes('RF') || normalized.includes('RAISED')) return 'SORF';
    if (normalized.includes('FF') || normalized.includes('FLAT')) return 'SOFF';
    return 'SORF'; // Default to RF
  }

  // Blind variants
  if (normalized.includes('BLIND') || normalized === 'BL') {
    if (normalized.includes('RTJ')) return 'BLRTJ';
    if (normalized.includes('RF') || normalized.includes('RAISED')) return 'BLRF';
    if (normalized.includes('FF') || normalized.includes('FLAT')) return 'BLFF';
 ...
 */
export function normaliseRating(rating: string | number | null): string {
  if (!rating) return 'NR'; // No Rating

  const normalized = String(rating).toUpperCase().trim();

  // Remove "CLASS" prefix if present
  const cleaned = normalized.replace(/^CLASS\s*/i, '');
  ...
  const pnMatch = cleaned.match(/^PN(\d+)$/);
  if (pnMatch) {
    return `PN${pnMatch[1]}`;
  }
...
export function normaliseStandard(standard: string | null): string {
  ...
  if (normalized.includes('JIS')) {
    const match = normalized.match(/JIS\s*([A-Z]\d+)/);
    if (match) {
      return `JIS${match[1]}`;
    }
  }
 ...
 */
export function buildFlangeSku(flange: Flange): string {
  const npsPart = normaliseNpsForSku(flange.nps_inch);
  const typePart = normaliseFlangeType(flange.flange_type);
  const ratingPart = normaliseRating(flange.pressure_rating);
  const stdPart = normaliseStandard(flange.standard);
  const specPart = normaliseMaterialSpec(flange.material_spec);

  return `FLNG-${npsPart}-${typePart}-${ratingPart}-${stdPart}-${specPart}`;
}
 ...
 */
export function validateFlangeSku(sku: string): boolean {
  // Pattern: FLNG-{NPS}-{TYPE}-{RATING}-{STANDARD}-{GRADE}
  const pattern = /^FLNG-[\d_]+IN-[A-Z0-9]+-[A-Z0-9]+-[A-Z0-9]+-[A-Z0-9]+$/;
 ...
 */
export function parseFlangeSku(sku: string): {
  nps: string;
  type: string;
  rating: string;
  standard: string;
